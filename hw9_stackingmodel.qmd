...
```{r}
library(xgboost)
dtrain <- xgb.DMatrix(data = as.matrix(train_data[,-which(names(train_data) == "Y")]), label = train_data$Y)
dtest <- xgb.DMatrix(data = as.matrix(test_data[,-which(names(test_data) == "Y")]), label = test_data$Y)
```
...
predictions <- predict(boost_model, dtest)


rmse_log <- sqrt(mean((predictions - test_data$Y)^2))
