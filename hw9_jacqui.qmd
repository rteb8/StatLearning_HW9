...
train = read.csv("C:/jacqu/OneDrive/Documents/MSDS/datasets/train.csv")
...
```{r}
mx = glmnet::makeX(
  data_train = train %>% select(-SalePrice),
  data_test = test)

mx.train = mx$x
y.train = train %>% pull(SalePrice)
mx.test = mx$xtest
...
enet_model = cv.glmnet(mx.train,
                       y.train,
                       alpha = best_alpha,
                       lambda = best_lambda)

preds = data.frame(predict(enet_model, mx.test, s="lambda.min"))
